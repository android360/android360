...
(function($){
	...
	var _process = function(e, data, options) {
		...
		// 添加标题如果需要的话
		if (options.header)
			html +=	'<form>' + '<input data-type="search" id="searchForCollapsibleSetChildren">' + '</form>';
		...
		for (var i=options.offset; i<feeds.entries.length; i++) {
			...
			if (options.date && pubDate) rowArray[rowIndex]['html'] += '<div id="rsspudate">'+ pubDate +'</div>'
			if (options.content) {
		...
		$.each(rowArray, function(e) {
			...
			html += '<div class="rssRow data-role="collapsible" data-filtertext="filtertext" '+row+'">' + rowArray[e]['html'] + '</div>';
